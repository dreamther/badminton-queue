...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 text-slate-100 overflow-hidden relative">
      {/* Global Banner for Selected Player */}
      {showGlobalBanner && selectedPlayerForMove && (() => {
        const p = players.find(p => p.id === selectedPlayerForMove);
        if (!p) return null;
        return (
          <div className="absolute top-0 inset-x-0 z-[60] animate-in slide-in-from-top-full bg-indigo-600/95 backdrop-blur shadow-xl border-b border-indigo-500/50 flex flex-col sm:flex-row items-center justify-between px-4 py-2 sm:py-3 gap-2">
             <div className="flex items-center gap-3">
                 <PlayerAvatar identifier={p.name} className="w-8 h-8 shrink-0 rounded-full shadow-sm" />
                 <div><div className="font-bold text-white text-sm">已選取 {p.name}</div><div className="text-xs text-indigo-200">請切換分頁並點擊空位放置</div></div>
             </div>
             <div className="flex items-center gap-2">
                 <button onClick={() => deletePlayer(p.id)} className="bg-red-500/80 hover:bg-red-400 text-white text-xs px-3 py-2 rounded flex items-center gap-1 transition-colors"><LogOut className="w-3.5 h-3.5"/>早退</button>
                 <button onClick={() => setSelectedPlayerForMove(null)} className="bg-indigo-800 hover:bg-slate-700 text-white text-xs px-4 py-2 rounded transition-colors">取消</button>
             </div>
          </div>
        );
      })()}
      ...
      <header className="px-4 py-3 bg-slate-950 border-b border-slate-800 flex items-center justify-between z-20 shrink-0">
          <div className="flex items-center justify-between mb-1 w-full flex-1">
            <div className="flex items-center gap-3">
